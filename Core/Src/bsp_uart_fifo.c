...
UART_T *ComToUart(COM_PORT_E _ucPort)
{
	...
	{
		Error_Modbus_Handler(__FILE__, __LINE__);
		return 0;
	}
}
...
static void UartSend(UART_T *_pUart, uint8_t *_ucaBuf, uint16_t _usLen)
{
	...
	for (i = 0; i < _usLen; i++)
	{
		...
		while (1)
		{
			...
			{
				// 如果发送缓冲区中的字节数等于发送缓冲区的大小，则使能发送中断
				if((_pUart->uart->CR1 & USART_CR1_TXEIE_TXFNFIE) == 0) //
				{
					SET_BIT(_pUart->uart->CR1, USART_CR1_TXEIE_TXFNFIE);// txt interrrupt enable  //FIFO mode is enabled by setting FIFOEN in USART_CR1 register bit 29
				}  
			}
		}
		...
		// Disable interrupts before the write, because the write
		// to the UART can take a long time.
		DISABLE_INT();
		// Check if the write will fit in the Tx buffer.
		if (++_pUart->usTxWrite >= _pUart->usTxBufSize)
		{
			// Wrap around the buffer.
			_pUart->usTxWrite = 0;
		}
		// Increase the count of bytes in the Tx buffer.
		_pUart->usTxCount++;
		// Enable interrupts after the write, because the CPU
		// can now be woken up by the UART.
		ENABLE_INT();
	}
